...
class Update
{
    ...
    protected const PHP_DOCKERFILE = "setup/Dockerfile";
    protected const PHP_DOCKERFILE_TARGET = "/usr/local/lib/doil/server/php/Dockerfile";
    ...
    public function run(string $repo, array $tags): void
    {
        ...
        foreach ($tags as $tag) {
            ...
            if (!$this->filesystem->exists($this->home_dir . "/" . self::BUILD_FILE_NAME)) {
                ...
                if (!$full_update) {
                    $this->writer->beginBlock($this->output, "Copying Doil");
                ...
                if ($php_container_update) {
                    $this->filesystem->copy(
                        self::DOIL_TARGET_DIR . "/" . self::PHP_DOCKERFILE,
                        self::PHP_DOCKERFILE_TARGET
                    );
        ...
        $this->output->writeln("\nYour doil installation is up to date.");
