...
ini_max_execution_time_apache2:
  cmd.run:
    - name: sed -i "/max_execution_time*/c max_execution_time = 3600" /etc/php/8.1/apache2/php.ini

ini_max_execution_time_cli:
  cmd.run:
    - name: sed -i "/max_execution_time*/c max_execution_time = 3600" /etc/php/8.1/cli/php.ini
...
update_alternatives_php:
  cmd.run:
    - name: update-alternatives --set php /usr/bin/php8.1 &>/dev/null
